// Full test: 1 day with combat, death, resurrection
...
function exportSnapshot() {
    ...
    const filename = `chronicle-seed${SEED}-scale${SCALE}-combat-test-${new Date().toISOString().split("T")[0]}.json`;
...
async function main(): Promise<void> {
    ...
    console.log(`characters=${STATE.characters.length} locations=${STATE.locations.length}`);
    ...
    console.log(`\nAction counts:`);
    Object.entries(actionCounts).sort((a, b) => b[1] - a[1]).forEach(([name, count]) => {
        console.log(`  ${name}: ${count}`);
    });

    // Sample last 20 actions
    console.log(`\nLast 20 actions:`);
    for (const aid of STATE.actions.slice(-20)) {
        const a = STATE.entities[aid] as ActionView;
        console.log(`  [d${dayOf(a.timestamp)} ${hourOf(a.timestamp).toString().padStart(2, "0")}h] ${a.gloss ?? a.name}`);
    }
